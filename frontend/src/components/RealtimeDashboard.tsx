...
// This dashboard drives a simulated paper-trading engine (prices are a seeded,
// mean-reverting random walk - see backend/app/routers/realtime_strategy.py),
// not a live market data feed. Shown wherever strategy status is surfaced so
// it can't be mistaken for real trading.
const SimulatedDataBanner: React.FC<{ compact?: boolean }> = ({ compact }) => (
  <div style={{
    display: 'flex',
    alignItems: 'center',
    gap: 8,
    padding: compact ? '4px 10px' : '10px 16px',
    borderRadius: 8,
    background: '#fff7e6',
    ...
    <span>{compact ? 'Simulated data' : 'Simulated paper trading — prices are generated by a seeded random walk, not a live market feed.'}</span>
  </div>
);
